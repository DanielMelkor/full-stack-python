...
def cargar_datos(lineas_archivo):
    ...
    for archivo in lineas_archivo:

        info = archivo.split(',')
        titulo = info[0]
        popularidad = info[1]
        voto_promedio = info[2]
        cantidad_votos = info[3]
        nuevo_generos = info[4].split(';')

        #print(info[3])
        #lista_genero = generos

        for genero in nuevo_generos:
            ...
            if genero in peliculas_por_genero:
                ...
                peliculas_por_genero(nuevo_generos).append(titulo)
